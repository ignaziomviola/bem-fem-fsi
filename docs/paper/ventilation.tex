...
antisymmetry, solved for rather than imposed, to \num{2.4e-14}. Ventilation removes
\SI{44}{\percent} of the lift and moves the centre of pressure to $0.19c$,
against the $3c/16$ of a supercavitating section. The computed mean cavity closure
angle lies within \SI{4}{\percent} of the value measured at washout, although its
aspect-ratio trend does not, and bi-stability spans ten degrees of incidence. Softening the strut deepens its suction margin by a
third and moves the operating point across the boundary beyond which a
ventilated cavity is sustained, so a lifting surface's load limit can be assessed
together with its structural response.
\end{abstract}
...
to measure the angle photographically: the cavity closes further aft as the Froude
number rises, and the jet sweeps from upstream towards the tip as the closure line
tapers.

At the condition of the published measurement - $\alpha = \ang{20}$,
$\Fnh = 1.5$, $\ARh = 1$ - the computed angle is \ang{39.1} to \ang{45.8} across
six to sixteen spanwise stations and \ang{10} to \ang{14} chordwise panels,
centred on about \ang{42.5} and insensitive to the chordwise resolution to
\ang{0.4}. The measured mean is \ang{40.75} and the proposed criterion is
\ang{45}, so the agreement is \SI{4}{\percent} at the published condition.

Figure~\ref{fig:closure}b makes the comparison at each case's own washout Froude
number rather than at a common one, which is where the angle was measured: the
Froude number is iterated to \eqref{eq:washout} evaluated on that case's computed
lift, so each point sits on its own predicted washout boundary. Over four
incidences and three immersed aspect ratios the mean is \ang{41.08} against the
measured \ang{40.75}, which is agreement to \SI{0.8}{\percent}. That agreement of
the mean conceals a disagreement that matters more, and it is reported rather than
verified: the computed angle is \ang{42.2} to \ang{46.6} at $\ARh = 1$, but
\ang{19.9} to \ang{21.4} at $\ARh = 0.5$ and \ang{57.4} to \ang{59.9} at
$\ARh = 1.5$, a factor of three across the range, where the measurement found the
angle tightly clustered for every incidence and aspect ratio. The mechanism is
geometric and can be named exactly. The angle is taken between the closure line
and the flow, so its tangent is the ratio of the cavity's depth extent to its
chordwise spread; the depth extent is the immersion while the chordwise spread is
set by the cavitation number and the sectional incidence, neither of which depends
on the immersion at fixed $\Fnh$ and $\alpha$. The angle therefore inherits the
aspect ratio directly. That the measured angle does not implies that the physical
cavity length scales with immersion, which this model does not reproduce, and the
\ang{45} criterion is consequently trustworthy here only near the aspect ratio at
which it was calibrated.

One further limitation belongs where the diagnostic is used. At high Froude
number, where the cavity is long at every depth, the angle becomes sensitive to
the spanwise resolution, drifting from \ang{47} to \ang{29} between six and
fourteen stations. The cause is the pinched immersed tip: the mesh builder pins
the span-end section to a point, the suction spike there is a consequence of that
pinch rather than of the flow, and the spuriously long cavity it produces has
heavy leverage on a fit of depth against closure position. Weighting the fit by
station area does not cure it, because the pinched station's area is not small.
The blunt tip of the experiments, and with it base ventilation and the aerated
...
\begin{figure}[t]
  ...
  \caption{The cavity closure line and its mean angle from the horizontal:
    ...
    $2\Phi$; (b) the mean closure angle against incidence for three immersed
    aspect ratios, each point at its own predicted washout Froude number, with the
    proposed stability criterion and the value measured at washout.}
  \label{fig:closure}
\end{figure}
...
added-mass ratio on a frozen cavity is $4.05$; the wet fundamental period is
resolved by $7.6$ steps at the time step used, which is above the resolution
constraint but not comfortably.
Figure~\ref{fig:transient} marches the coupled system from a wetted static
equilibrium through an inception event triggered by injecting air at the junction
of the leading edge and the free surface, which is the perturbation route of the
experiments and fires below the stall angle. The cavity depth reaches the tip
within one step while the length grows linearly at the imposed rate limit. The
lift falls from its wetted equilibrium of $0.2187$ to a mean of $0.1264$ over the
last third of the march, a ratio of $0.578$ against the $0.575$ that the steady
solve gives at the same incidence, so the marched and steady load losses agree to
half a per cent. The tip deflection grows from $1.21$ to a peak of
\num{2.19e-3} chords, an overshoot of $1.81$ on the static ventilated
deflection.
...
added-mass ratio is $4.05$, and the resulting wet period is resolved by $7.6$
...
$0.030$ at $0.05$, so the amplitude is bounded once the front is resolved.
Figure~\ref{fig:transient} uses $0.2$, the fastest of the three, because only at
that rate does the cavity reach its equilibrium extent within the march and the
marched load loss become comparable with the steady one; the price is the visible
ringing and two isolated steps, near $s = 1.0$ and $s = 2.3$, at which the lift
drops out and recovers within one step.
So far this is ordinary.
...
because here the cavity length is solved rather than assumed - though the angle
inherits the immersed aspect ratio directly, where the measurement found it almost
independent of it, and the stability criterion is therefore trustworthy near the
aspect ratio at which it was calibrated and not far from it. Wetted and
